...
def create_model(args, env):
    ...
    if args.algo == 'ppo':
        ...
        model = SAC("MlpPolicy", env, verbose=1, learning_rate=args.lr, gradient_steps=args.gradient_steps)
    else:
...
def plot_results(log_folder, title="Learning Curve"):
    ...
    y_avg = moving_average(y, window=50)
    # Truncate x
    x_avg = x[len(x) - len(y_avg) :]

    fig = plt.figure(title)
    plt.plot(x, y)
    plt.plot(x_avg, y_avg)
    plt.legend(["Reward", "50-episode average"])
